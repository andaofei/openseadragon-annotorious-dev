...
annotorious.Popup = function (annotator) {
    ...
    /** @private **/
    this._radio = goog.dom.query('.label_color_active', this.element)[0];

    /** @private **/
    this._text = goog.dom.query('.annotorious-popup-text', this.element)[0];

    /** @private **/
    this._text2 = goog.dom.query('.annotorious-popup-text2', this.element)[0];

    /** @private **/
    this._text3 = goog.dom.query('.annotorious-popup-text3', this.element)[0];
    ...
    var btnEdit = goog.dom.query('.annotorious-popup-button-edit', this._buttons)[0]; // 编辑
    var btnDelete = goog.dom.query('.annotorious-popup-button-delete', this._buttons)[0]; // 删除
...
annotorious.Popup.prototype.setAnnotation = function (annotation) {
    ...
    // console.log(annotation, 'setAnnotation');

    if (annotation.color)
        goog.style.setStyle(this._radio, 'background', annotation.color)
    else
        goog.style.setStyle(this._radio, 'background', '#eee');

    if (annotation.text)
        this._text.innerHTML = annotation.text.replace(/\n/g, '<br/>');
    else
        this._text.innerHTML = '<span class="annotorious-popup-empty">暂无备注</span>';

    if (annotation.label)
        this._text2.innerHTML = annotation.label.replace(/\n/g, '<br/>');
    else
        this._text2.innerHTML = '<span class="annotorious-popup-empty"></span>';

    if (annotation.name)
        this._text3.innerHTML = annotation.name.replace(/\n/g, '<br/>');
    else
        this._text3.innerHTML = '<span class="annotorious-popup-empty">无</span>';
